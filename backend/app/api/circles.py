...
def api_create_circle(circle_in: CircleCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    circle = create_circle(db, name=circle_in.name, description=circle_in.description, owner_id=user.id)
...
def api_join_circle(circle_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    circle = join_circle(db, UUID(circle_id), user.id)
    if not circle:
