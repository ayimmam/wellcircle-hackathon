...
| C3 | **Weekly circle digest via bot** — the Render bot already messages users; send each circle a Sunday summary ("Sara topped your circle with 120 pts") | `weekly_points` leaderboard exists but nobody sees it unless they open the app | Low: one bot job + one backend query endpoint | Yes |
