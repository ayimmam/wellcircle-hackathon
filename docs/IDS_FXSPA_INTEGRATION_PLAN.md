...
we have (`CLAUDE.md`: Vercel skips the scheduler; Render runs it as a single-instance Background Worker). It already has a
...
- Telegram `403 Forbidden: bot was blocked by the user` ⇒ `undeliverable` immediately (no retries),
  and the booking lands on the **staff call list** — a human phones them. Because Render is capped
  at 1 instance, there is no concurrent-drain race; the `FOR UPDATE SKIP LOCKED` claim on pending rows
...
| Vercel cold start delays confirmations | Late confirmations near the deadline | Critical loops live on always-on Render; existing `keep_warm` helps |
...
POST   /api/cron/outbox-drain                      # CRON_SECRET (Render fallback)
